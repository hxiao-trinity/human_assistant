...
class OllamaMCPClient:
    def __init__(self, model: str, server_url: str):
        ...
        self.system_prompt = (
            ...
            "- When being asked to do make actions to the files, only use ingest_my_files tool.\n"
            "- Please always give tool call as tool_calls object.\n"
            "- When I use /vc, use correct_command function to pass it into the LLM.\n"
        )
    ...
    def call_tool_direct(self, name: str, arguments: dict) -> str:
        async def _do_execute():
            async with streamablehttp_client(url=self.server_url) as (read, write, _):
                async with ClientSession(read, write) as session:
                    await session.initialize()
