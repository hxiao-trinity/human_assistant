...
# (adapted to also work in text-only mode when there are no numeric overlays)
# =============================================================================
...
class CaptureScreenWithNumbers(Workflow[dict]):
    """
    Screen capture workflow with OCR and number/text mapping.

    On macOS / Win11 with numeric overlays:
      - it will try to build mappings number -> label.

    On Win10 (no numeric overlays):
      - it falls back to pure text-based mappings:
        each OCR text box becomes a mapping entry with center_x/center_y.
    """
    ...
    async def analyze_screen(self) -> dict:
        """Capture screenshot and perform OCR with number-to-text / text-only mapping."""
        ...
        # Run OCR
        ocr = get_ocr()
        ocr_result = ocr.ocr(screenshot_path)  # (page -> list of [box, (txt, conf)])
        ...
        print("\n===== RAW OCR RESULT =====")
        # If needed, you can uncomment this to print a JSON-safe version:
        # print(json.dumps(numpy_to_list(ocr_result), indent=2))
        pprint(ocr_result)
        print("==========================\n")

        mappings: List[dict] = []
        number_entries: List[dict] = []
        ...
        # =======================================================
        # IMPORTANT: TEXT-ONLY FALLBACK (for Win10 / no numbers)
        # =======================================================
        if not final_mappings:
            logger.info(
                "[CaptureScreenWithNumbers] No numeric mappings found; "
                "falling back to text-only mappings."
            )
            for t in text_entries:
                clean = (t.get("text") or "").strip()
                if not clean:
                    continue
                # Skip pure punctuation or single weird chars
                if len(clean) == 1 and not clean.isalnum():
                    continue

                final_mappings.append(
                    {
                        "number": t.get("digits") or None,
                        "text": clean,
                        "center_x": t["center_x"],
                        "center_y": t["center_y"],
                    }
